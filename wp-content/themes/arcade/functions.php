 ...
 * @since 1.0.0
 */
$bavotasan_theme_data = wp_get_theme( 'arcade' );
...
define( 'BAVOTASAN_THEME_CODE', 'arc' );
...
function bavotasan_setup() {
	...
	register_nav_menu( 'social', __( 'Social', 'arcade' ) );
...
a, .page-header .navigation a { color: <?php echo esc_attr( $bavotasan_theme_options['link_color'] ); ?>; }
a:hover { color: <?php echo esc_attr( $bavotasan_theme_options['link_hover_color'] ); ?>; }
body { <?php echo bavotasan_prepare_font( $bavotasan_theme_options['main_text_font'] ); ?>; font-size: <?php echo esc_attr( $bavotasan_theme_options['main_text_font_size'] ); ?>px; }
#site-title a { <?php echo bavotasan_prepare_font( $bavotasan_theme_options['site_title_font'] ); ?>; }
.only-on-home.no-fittext #site-title a { font-size: <?php echo esc_attr( $bavotasan_theme_options['site_title_font_size'] ); ?>px; }
main, #home-page-widgets, .home-jumbotron, #site-navigation { color: <?php echo esc_attr( $bavotasan_theme_options['main_text_color'] ); ?>; }
h1, h2, h3, h4, h5, h6, h1 a, h2 a, h3 a, h4 a, h5 a, h6 a { color: <?php echo esc_attr( $bavotasan_theme_options['headers_color'] ); ?>; <?php echo bavotasan_prepare_font( $bavotasan_theme_options['headers_font'] ); ?>; }
.entry-title, .entry-title a { <?php echo bavotasan_prepare_font( $bavotasan_theme_options['post_title_font'] ); ?>; font-size: <?php echo esc_attr( $bavotasan_theme_options['post_title_font_size'] ); ?>px; }
.entry-meta, .entry-meta a { <?php echo bavotasan_prepare_font( $bavotasan_theme_options['post_meta_font'] ); ?>; font-size: <?php echo esc_attr( $bavotasan_theme_options['post_meta_font_size'] ); ?>px; }
<?php if ( $custom_css = get_option( 'arcade_custom_css' ) ) { ?>
/* Custom CSS */
...
function bavotasan_add_js() {
	...
	wp_enqueue_script( 'theme_js', BAVOTASAN_THEME_URL .'/library/js/theme.js', array( 'bootstrap', 'fillsize', 'arctext' ), '', true );
...
function bavotasan_font_url() {
	...
	$selected_fonts = array_unique( array(
	...
	return ( $font_string ) ? add_query_arg( 'family', $font_string, '//fonts.googleapis.com/css' ) : '';
}
...
function bavotasan_widgets_init() {
	...
	register_sidebar( array(
		...
		'id' => 'sidebar',
		'description' => __( 'This is the first sidebar. It won\'t appear on posts/pages that are set to full width.', 'arcade' ),
	...
	register_sidebar( array(
		'name' => __( 'Second Sidebar', 'arcade' ),
		'id' => 'second-sidebar',
		'description' => __( 'This is the second sidebar. It won\'t appear unless you have added at least one widget.', 'arcade' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Extended Footer', 'arcade' ),
		'id' => 'extended-footer',
		'description' => __( 'This is the extended footer. You can set the number of columns in the customizer. Add at least one widget to make it appear.', 'arcade' ),
	...
	register_sidebar( array(
		'name' => __( 'Footer Notice', 'arcade' ),
		'id' => 'footer-notice',
		'description' => __( 'This is the footer notice area. It works best with a copyright notice or a banner ad.', 'arcade' ),
...
function bavotasan_pagination() {
	...
	<nav class="navigation clearfix" role="navigation">
		<div class="sr-only"><?php _e( 'Posts navigation', 'arcade' ); ?></div>
...
function is_bavotasan_full_width() {
	$single_layout = ( is_singular() ) ? get_post_meta( get_the_ID(), 'arcade_single_layout', true ) : '';
	if ( 'on' == $single_layout )
		return true;
}
...
/**
 * Create grid for grid template
 *
 * @since 1.0.2
 */
function bavotasan_grid_template() {
	global $bavotasan_grid_query, $paged;

	/**
	 * You can overwrite the following arguments in your child theme's functions.php by
	 * hooking into the 'bavotasan_grid_args' and returning a custom array.
	 *
	 * https://codex.wordpress.org/Function_Reference/add_filter
	 */
	$args = apply_filters( 'bavotasan_grid_args', array(
		'paged' => $paged,
		'posts_per_page' => 9,
		'ignore_sticky_posts' => 1,
	) );

	$bavotasan_grid_query = new WP_Query( $args );

	if ( $bavotasan_grid_query->have_posts() ) :
		?>
		<div class="grid-template row">
		<?php
		while ( $bavotasan_grid_query->have_posts() ) : $bavotasan_grid_query->the_post();
			?>
			<div id="post-<?php the_ID(); ?>" <?php post_class( 'col-md-4' ); ?>>
				...
				<div class="grid-wrapper clearfix">
				    ...
				    <div class="entry-content">
					    <?php
					    global $bavotasan_custom_excerpt_length;
					    $bavotasan_custom_excerpt_length = 15;
					    the_excerpt();
					    ?>
				    </div><!-- .entry-content -->
				</div>
